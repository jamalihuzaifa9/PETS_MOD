            ...
            with tf.variable_scope(self.name):
                ...
                for i, layer in enumerate(self.layers):
                    with tf.variable_scope("Layer%i" % i):
                        layer.construct_vars()
                        #z = layer.get_vars()
                        #zz = [z[i][j][None] for i in range(len(z))]
                        #optvars.extend(zz)
                        optvars.extend(layer.get_vars())
        optvars.extend([self.max_logvar, self.min_logvar])
        return optvars[:-2]
        # return self.optvars[:-2]
